...
def run_zscore_detection(bgr, rois, window, z_thr, min_area=4.0, min_circularity=0.55,
    ...
    lighting/lens artifact doesn't always look like a clean thin arc --
      ...
      2. Concentric-band check using the ROI's own known center: for
         ...
         of variation <= max_concentric_band_cv), the blob is rejected
         regardless of width. This is what a "moon"/crescent-shaped
         reflection near the lens center needs: it has two boundary
         edges at DIFFERENT radii (an annular sector, not one clean arc),
         so a single circle fit doesn't explain it well -- but it's
         still obviously centered on the lens axis, which real
         contamination has no reason to be. Checked BEFORE the glue/
         thread width split, since a crescent artifact can be wide
         enough to otherwise be routed straight to glue without ever
         being shape-tested at all.
    ...
    for lbl in candidate_labels:
        ...
        if nearest_roi is not None:
            dists = np.hypot(all_pts[:, 0] - nearest_roi["cx"], all_pts[:, 1] - nearest_roi["cy"])
