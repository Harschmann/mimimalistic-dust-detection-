...
def run_zscore_detection(bgr, rois, window, z_thr, min_area=4.0, min_circularity=0.55,
    ...
    A "does this fit a straight line better than a circle" safeguard was
    added and then removed: a straight line IS the infinite-radius limit
    of a circle, so it was meant to stop an ordinary straight thread
    fragment from spuriously fitting some large circle -- but the same
    check made the arc test too strict for genuinely curved, SHORT arcs
    (a short visible segment doesn't sag far from straight even when it
    truly is part of a circle), so real arcs that were previously
    rejected correctly started leaking through as thread. Given that a
    real thread tracing a clean circle by chance is rare, while a real
    lens/coating arc doing so is the norm, the test is deliberately
    biased toward calling a clean circle-fit an arc -- occasionally
    misclassifying a genuinely straight thread fragment as arc is the
    acceptable side of that tradeoff, not the other way around.
