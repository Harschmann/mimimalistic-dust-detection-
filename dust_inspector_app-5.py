...
def run_zscore_detection(bgr, rois, window, z_thr, min_area=4.0, min_circularity=0.55,
                          ...
                          max_arc_radius_ratio=80.0, gap_bridge_px=7.0,
    ...
    size to the blob, being mistaken for an arc. A straight line is also
    the infinite-radius limit of a circle, so with a generous upper cap a
    genuinely straight thin fragment could technically fit some big
    circle too -- line_straightness below guards against that by
    requiring the circle to explain the points meaningfully better than
    a straight line does before calling it an arc.)

    A fragment-linking pass that tried to reconnect thread pieces
    separated by a gap wider than gap_bridge_px was added and then
    removed: it did an O(n^2) pairwise comparison across every raw
    fragment, which froze the pipeline when thresholds are set loose
    enough to produce hundreds/thousands of noise fragments, and it also
    pulled genuinely separate dust specks into nearby thread lines when
    they happened to sit roughly on-axis, which is worse than leaving a
    knot in a thread occasionally mislabeled dust. Detection is back to
    directly following what actually clears the z-score threshold, with
    only the small isotropic gap_bridge_px dilation (bridges a few px,
    every direction, same as before) -- no larger direction-aware
    linking. A stretch that never clears z_thr_low ANYWHERE along its
    length still can't be recovered -- there's no pixel signal there to
    work with. That's a real limit of any threshold-based method; the
    levers for a stretch like that are z_thr_low itself (lower it) and
    physical imaging (exposure/lighting), not shape logic downstream of it.
    ...
    # gap_bridge_px=0 means EXACTLY zero dilation -- no forced minimum.
    # A previous version always ran at least 1 iteration regardless of
    # this setting, so "0" didn't actually mean "off"; that's fixed here.
    ...
    for lc in link_contours:
        ...
        if not strong_contours or strong_area < 1:
            ...
            continue
        # min_area is the user's real size floor, but until now it was only
        # checked against `area` (the full weak+strong hysteresis region) --
        # the ACTUAL reported diameter/width below comes from strong_part
        # alone. A blob with a big diffuse weak halo (area passes min_area)
        # but almost no real strong-pixel core (strong_area near 0) was
        # slipping through and reporting its size from that near-empty
        # core -- a "1px" dust speck, or a fake thin "thread" whose width
        # (also strong-only) rounds to ~0 and makes even a short faint
        # smear look wildly elongated by ratio. Applying the SAME min_area
        # floor to the strong core closes that loophole.
        ...
        if strong_ys.size >= 10:
            ...
            # length is genuinely thin. A high percentile still correctly
            # flags a UNIFORMLY thick object (most points are wide) as
            # glue, while not letting one small thick patch decide the
            # call for an otherwise-thin thread.
...
class DustInspectorApp:
    ...
    def _build_main_overlay(self):
        ...
        for roi in self.rois:
            ...
            half = r + margin
            top_left = (int(round(cx - half)), int(round(cy - half)))
            bottom_right = (int(round(cx + half)), int(round(cy + half)))
            cv2.rectangle(disp, top_left, bottom_right, (0, 255, 0), 2)
    ...
    def _build_result_strip(self):
        ...
        for roi in self.rois:
            ...
            cv2.rectangle(crop, (0, 0), (crop.shape[1] - 1, crop.shape[0] - 1), (0, 255, 0), 2)
            crops.append(crop)
        if not crops:
    ...
    def resume_feed(self):
        ...
        showing) to the normal full frame. Only meaningful after Start
        has frozen it; harmless no-op if the feed is already live."""
    ...
    def _run_inspection_thread(self, frame, rois_snapshot, barcode):
        ...
        try:
            _binary, blobs, _stats, _dbg = run_zscore_detection(
                ...
                gap_bridge_px=s["gap_bridge_px"], z_thr_low=s["z_thr_low"])
        except Exception:
            blobs = []
    ...
    def _run_pipeline_debug_thread(self, frame, rois_snapshot):
        ...
        try:
            _binary, _blobs, _stats, debug_images = run_zscore_detection(
                ...
                s["min_aspect_ratio"], s["max_thin_width_px"], s["max_arc_fit_residual"],
                max_arc_radius_ratio=s.get("max_arc_radius_ratio", 80.0),
                gap_bridge_px=s["gap_bridge_px"], z_thr_low=s["z_thr_low"], debug=True)
        except Exception:
            debug_images = None
