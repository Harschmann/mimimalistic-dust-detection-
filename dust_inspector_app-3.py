...
DEFAULT_SETTINGS = {
    ...
    "max_arc_radius_ratio": 80.0,
    "gap_bridge_px": 11.0,
    "thread_link_radius_px": 35.0,
    "z_thr_low": 1.5,
...
def _fit_line_residual(pts):
    ...
    Used only to DISAMBIGUATE the arc test below, not to classify shape
    on its own: a straight line is the limiting case of a circle with
    infinite radius, so the algebraic circle fit (_fit_circle) can
    return a very large radius with a deceptively LOW residual for data
    that's simply straight, not curved -- without this check, raising
    max_arc_radius_ratio to accommodate genuinely large lens-reflection
    curvature also made it easier for an ordinary straight (or nearly
    straight) thread segment to spuriously "fit" some large circle and
    get misclassified as an arc. Requiring the circle to fit MEANINGFULLY
    BETTER than a straight line (i.e. line_residual clearly worse than
    circle_residual) is what actually distinguishes "this has genuine
    curvature matching real lens/coating geometry" from "this is just
    straight, and any sufficiently large circle technically fits it too".
    """
...
def _link_thread_fragments(raw, thread_link_radius_px, angle_tol_deg=35.0, min_elong_ratio=2.5):
    """Bridges genuinely separate hysteresis fragments that are likely
    pieces of the SAME thread, before gap-bridging/classification run.

    This is a SECOND, direction-aware linking pass, on top of (not a
    replacement for) the existing small isotropic gap-bridging dilation
    below. That dilation only bridges pixel gaps up to a few px wide,
    isotropically, in every direction -- it can't reconnect fragments
    separated by a real, wider no-signal gap (a stretch of the thread
    that dipped below even the weak hysteresis threshold), and being
    isotropic it also can't tell "this gap continues the thread's own
    direction" from "this gap heads off toward an unrelated blob".

    Without this, a thread that has one higher-contrast segment (e.g. a
    knot, a twist, or wherever the lighting happened to catch it best)
    and a separated, fainter remainder gets left as two SEPARATE blobs.
    The higher-contrast segment, judged on its own, can easily be small
    and round enough to pass the DUST test -- which is exactly the "part
    of the thread is being shown as dust" failure mode.

    For every pair of raw connected components (compared via their
    CONTOUR points, not every filled pixel -- much cheaper, and it's the
    boundary that determines how close two shapes actually get), this
    bridges them (draws a thin connecting line directly into the mask)
    only when BOTH:
      1. proximity: the TRUE closest pair of points between the two
         contours (a full nearest-point search, not just each shape's
         PCA-projected extremes) is within thread_link_radius_px.
         Using only PCA-extreme "endpoints" was tried first and doesn't
         work for a near-round component -- a knot's principal axis is
         nearly arbitrary (its two eigenvalues are close), so the two
         points it picks as "endpoints" aren't reliably the ones facing
         the neighboring fragment, which silently underestimated
         proximity and let real knot+tail pairs slip through unlinked.
      2. direction: for whichever component(s) are already elongated
         enough to have a trustworthy axis of their own (elongation
         ratio >= min_elong_ratio, judged from that component's OWN
         contour only, via PCA), the connecting line continues roughly
         along that axis (within angle_tol_deg) rather than heading off
         at a sharp angle.
    A round, compact fragment (elongation ratio below min_elong_ratio on
    both sides) is never bridged to another round fragment -- that
    guards against two separate, unrelated dust specks that simply
    happen to be near each other getting stitched into one blob.

    Returns a new mask (does not modify raw in place); downstream gap-
    bridging and classification run unchanged on the result.
    """
    n_labels, labels = cv2.connectedComponents(raw, connectivity=8)
    if n_labels <= 2:  # background + at most one component -- nothing to link
        return raw
    ...
    for lbl in range(1, n_labels):
        ...
        if len(pts) < 3:
            continue
        # cap contour point count for the O(n*m) nearest-pair search below
        # -- a long thread's full contour can run into the thousands of
        # points; a few hundred evenly-spaced samples is plenty to find
        # the true nearest approach to another fragment
        if len(pts) > 400:
        ...
        elong = float(np.sqrt(eigvals[0] / eigvals[1])) if eigvals[1] > 1e-9 else 1e9
        comps.append({"pts": pts, "axis": axis_dir, "elong": elong})
    ...
    for i in range(n):
        for j in range(i + 1, n):
            ...
            if ci["elong"] < min_elong_ratio and cj["elong"] < min_elong_ratio:
                continue  # neither side looks thread-like on its own -- don't guess
            diff = ci["pts"][:, None, :] - cj["pts"][None, :, :]
            d2 = np.einsum("ijk,ijk->ij", diff, diff)
            idx_flat = int(np.argmin(d2))
            ii, jj = np.unravel_index(idx_flat, d2.shape)
            dist = float(np.sqrt(d2[ii, jj]))
            if dist > thread_link_radius_px or dist < 1e-6:
                continue
            pa, pb = ci["pts"][ii], cj["pts"][jj]
            connect_dir = (pb - pa) / dist
            aligned = True
            for c in (ci, cj):
                if c["elong"] >= min_elong_ratio:
                    cos_ang = abs(float(np.dot(connect_dir, c["axis"])))
                    if np.degrees(np.arccos(np.clip(cos_ang, -1.0, 1.0))) > angle_tol_deg:
                        aligned = False
                        break
            if not aligned:
                continue
            cv2.line(bridged, (int(round(pa[0])), int(round(pa[1]))),
                     (int(round(pb[0])), int(round(pb[1]))), 255, 2)
    return bridged
...
def run_zscore_detection(bgr, rois, window, z_thr, min_area=4.0, min_circularity=0.55,
                          ...
                          max_arc_radius_ratio=80.0, gap_bridge_px=7.0, thread_link_radius_px=35.0,
                          z_thr_low=1.5, debug=False):
    ...
    Every surviving connected blob is then classified by SHAPE:
      ...
      - elongated (length/width >= min_aspect_ratio), thin
        (width <= max_thin_width_px), NOT a lens/reflection arc -> THREAD/FIBER
    ...
    Thread covers the full range that's actually shown up: thin single-
    strand fiber, slightly thicker strands, and any amount of curl/curve
    down to nearly straight for a short fragment -- shape alone doesn't
    reliably separate "thread" from anything else at this stage, so no
    attempt is made to split it further (a SCRATCH sub-category and a
    separate PITTING detector were both tried and removed: they produced
    more wrong labels than right ones on real module images).
    ...
    max_arc_radius_ratio caps how much bigger the fitted circle can be
    than the blob itself and still count as an arc -- this used to be a
    hardcoded 25.0, which was too tight: a real lens/module reflection's
    true curvature radius is often much larger than the short visible arc
    segment sitting in one ROI, so many genuine arcs were failing this
    cap and falling through to be labeled thread instead. It's now a
    tunable setting (default 80.0) so it can be raised to match the
    actual lens geometry without editing code. (The lower bound, 1.2,
    stays fixed -- it guards against a thread that's curled into nearly
    a full loop on itself, whose fitted circle would be comparable in
    size to the blob, being mistaken for an arc.)

    Two fixes address fragmented threads (one part of a real thread
    showing up as a separate "dust" blob, and faint stretches vanishing
    entirely):
      1. _link_thread_fragments runs BEFORE gap-bridging and reconnects
         raw hysteresis fragments that are likely pieces of the same
         thread (close together AND continuing each other's own
         orientation -- see its docstring), which the existing small
         isotropic gap-bridge dilation can't do across a wider genuine
         gap. This is what stops a thread's higher-contrast segment (a
         knot, a twist, wherever lighting caught it best) from being left
         as its own small, separately-classified fragment.
      2. Even after linking, the DUST test used to look at the STRONG
         core's shape alone -- so a round, high-contrast knot could still
         get called dust even while correctly linked into a longer raw
         region, because the dust test never looked at that larger
         region. whole_region_elongated now checks the FULL bridged raw
         region's own length/width first; if the whole thing is clearly
         elongated, the dust test is skipped entirely and it's routed to
         glue/arc/thread instead, regardless of what the strong core in
         isolation looks like.
    Neither fix can recover a stretch that never clears z_thr_low
    ANYWHERE along its length -- there's no pixel signal there to work
    with. That's a real limit of any threshold-based method; the levers
    for a stretch like that are z_thr_low itself (lower it) and physical
    imaging (exposure/lighting), not shape logic downstream of it.
    ...
    # Direction-aware linking FIRST, before the small isotropic gap
    # bridge below: reconnects fragments of the same thread that are
    # separated by a real gap wider than gap_bridge_px (see
    # _link_thread_fragments docstring) -- without this, a thread's
    # higher-contrast segment can be left as its own small, round,
    # independently-classified blob and get called dust.
    if thread_link_radius_px > 0:
        raw = _link_thread_fragments(raw, thread_link_radius_px)
    ...
    for lc in link_contours:
        ...
        # Whole-region elongation check, using the FULL bridged/hysteresis
        # region (`original`) rather than just the strong core -- this
        # decides whether we even ATTEMPT the dust (circular) test below.
        # Without this, a thread's higher-contrast segment (a knot, a
        # twist, wherever lighting caught it best) could get classified
        # as dust purely from ITS OWN strong-pixel shape, even though
        # directed fragment-linking / hysteresis / gap-bridging has
        # already attached it to a longer, genuinely elongated raw
        # region -- i.e. it's part of a thread, not an isolated speck,
        # regardless of what its own strong core looks like in isolation.
        raw_dist = cv2.distanceTransform(original, cv2.DIST_L2, 5)
        raw_width_px = 2.0 * float(raw_dist.max())
        whole_region_elongated = (raw_width_px > 1e-6) and (length_px / raw_width_px) >= min_aspect_ratio
        ...
        if (not whole_region_elongated) and circularity >= min_circularity and len(strong_contours) == 1:
        ...
        strong_ys, strong_xs = np.nonzero(strong_part)
        if strong_ys.size >= 10:
            # 70th percentile of local half-width, not the raw maximum --
            # a short, locally thicker patch (e.g. a knot on an otherwise
            # thin thread, now correctly linked into one blob by
            # _link_thread_fragments) could otherwise single-handedly
            # push width_px past max_thin_width_px even though most of
            # the object's length is genuinely thin. A high percentile
            # still correctly flags a UNIFORMLY thick object (most points
            # are wide) as glue, while not letting one small thick patch
            # decide the call for an otherwise-thin, multi-segment thread.
            width_px = 2.0 * float(np.percentile(dist[strong_ys, strong_xs], 70))
        else:
        ...
        if width_px > max_thin_width_px:
            ...
            if len(all_pts) >= 8:
                ...
                if fit_r is not None:
                    ...
                    # A straight line is the infinite-radius limit of a
                    # circle, so data that's simply straight can still
                    # produce a low norm_resid against SOME large circle
                    # -- especially now that max_arc_radius_ratio is
                    # generous enough to admit real lens-curvature radii.
                    # Requiring the circle to explain the points clearly
                    # better than a straight line does (line_straightness
                    # noticeably worse than max_arc_fit_residual) is what
                    # actually proves genuine curvature, not just "any
                    # big-enough circle technically fits a straight line".
                    line_resid = _fit_line_residual(all_pts)
                    line_straightness = line_resid / max(length_px, 1e-6)
                    if (norm_resid <= max_arc_fit_residual and 1.2 <= radius_ratio <= max_arc_radius_ratio
                            and line_straightness > max_arc_fit_residual):
                        is_arc = True
            if is_arc:
                rejected += 1
    ...
    if debug:
        ...
        debug_images["5 After hysteresis (before bridging)"] = cv2.cvtColor(raw_before_link, cv2.COLOR_GRAY2BGR)
        debug_images["6 After directed thread-linking"] = cv2.cvtColor(raw, cv2.COLOR_GRAY2BGR)
        debug_images["7 After gap-bridging"] = cv2.cvtColor(linked, cv2.COLOR_GRAY2BGR)
        ...
        debug_images["8 Final classified result"] = result_disp
...
class DustInspectorApp:
    ...
    def _run_inspection_thread(self, frame, rois_snapshot, barcode):
        ...
        try:
            _binary, blobs, _stats, _dbg = run_zscore_detection(
                ...
                max_arc_radius_ratio=s.get("max_arc_radius_ratio", 80.0),
                gap_bridge_px=s["gap_bridge_px"], thread_link_radius_px=s.get("thread_link_radius_px", 35.0),
                z_thr_low=s["z_thr_low"])
        except Exception:
            blobs = []
    ...
    def _run_pipeline_debug_thread(self, frame, rois_snapshot):
        ...
        try:
            _binary, _blobs, _stats, debug_images = run_zscore_detection(
                ...
                max_arc_radius_ratio=s.get("max_arc_radius_ratio", 80.0),
                gap_bridge_px=s["gap_bridge_px"], thread_link_radius_px=s.get("thread_link_radius_px", 35.0),
                z_thr_low=s["z_thr_low"], debug=True)
        except Exception:
            debug_images = None
    ...
    def _build_detection_tab(self, tab):
        ...
        self.arc_radius_ratio_var = tk.StringVar(value=str(self.settings.get("max_arc_radius_ratio", 80.0)))
        ...
        self._field(row3, "Max arc radius ratio", self.arc_radius_ratio_var, width=100).pack(side="left", padx=(0, 16))
        ctk.CTkLabel(card, text="A thin elongated blob is rejected as an arc (lens/coating reflection) only if it cleanly fits ONE circle much bigger than itself -- a real thread can curve too, but essentially never traces a perfect large arc, so curvature alone no longer disqualifies it. Lower the tolerance to be stricter about what counts as a clean circle fit. Max arc radius ratio caps how much bigger that fitted circle can be than the blob itself and still count as an arc -- raise this if real lens/coating reflections (large, gentle curvature) are getting called thread instead of being rejected; the true curvature radius is often much bigger than the short arc segment visible in one ROI.",
        ...
        self.thread_link_var = tk.StringVar(value=str(self.settings.get("thread_link_radius_px", 35.0)))
        ...
        self._field(row4, "Thread link radius (px)", self.thread_link_var, width=110).pack(side="left", padx=(0, 16))
        self._btn_primary(row4, "Save", self._save_detection_settings, width=100).pack(side="left", pady=(18, 0))
        ctk.CTkLabel(card, text="Three different fixes for a broken-up thread: Weak threshold recovers a FAINT tail that's still elevated in the z-score but below the main threshold (hysteresis: a weak pixel counts if it touches a strong one, same trick Canny edge detection uses). Gap bridge links pieces separated by a true GAP with no signal at all, in any direction, over a short distance. Thread link radius is a LONGER-range version of gap bridge, but direction-aware -- it only bridges two fragments when they're already elongated and the connecting line continues their own orientation, so it can close a wider real gap without stitching together unrelated nearby specks. This is also what stops a thread's higher-contrast segment (a knot, a twist) from being left as its own small blob and mislabeled dust once it's properly linked to the rest of the thread.",
    ...
    def _save_detection_settings(self):
        try:
            ...
            self.settings["max_arc_radius_ratio"] = float(self.arc_radius_ratio_var.get())
            self.settings["gap_bridge_px"] = float(self.gap_bridge_var.get())
            self.settings["thread_link_radius_px"] = float(self.thread_link_var.get())
