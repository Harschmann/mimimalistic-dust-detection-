...
ICONS_B64 = {
    ...
    "barchart_muted": (
        "iVBORw0KGgoAAAANSUhEUgAAAQAAAAEACAYAAABccqhmAAAACXBIWXMAAAsTAAALEwEAmpwYAAABFUlEQVR4nO3BMQEAAADCoPVP"
        "7WsIoAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
        "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
        "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
        "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA6AwBPAABo9vSmwAAAABJRU5ErkJggg=="
    ),
    "barchart_white": (
        "iVBORw0KGgoAAAANSUhEUgAAAQAAAAEACAYAAABccqhmAAAACXBIWXMAAAsTAAALEwEAmpwYAAABFUlEQVR4nO3BMQEAAADCoPVP"
        "7WsIoAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
...
class TrainingStudioApp:
    ...
    def _build_left_column(self, parent):
        ...
        ctk.CTkLabel(data_card, text="Point these at your existing storage/results/OK and storage/results/NG "
                                      ...
                                      "the defect folder just measures how well the model separates good from bad.",
    ...
    def _set_active_run(self, run):
        name = self.profile_var.get()
        self.profile_mgr.set_active_run(name, run["run_id"])
        self._refresh_run_history()
    ...
    def start_training(self):
        ...
        self.profile_mgr.remember_last_settings(profile, good_dir, defect_dir, model_key)

        run_id = f"{model_key}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        run_dir = os.path.join(self.profile_mgr.profile_dir(profile), run_id)
        ...
        self.log_box.configure(state="disabled")
        self._log(f"Starting {model_key} training for profile '{profile}'.")

        threading.Thread(
            target=run_training_job,
            args=(profile, good_dir, defect_dir, plugin, run_dir, max_epochs,
                  lambda msg: self.root.after(0, self._handle_progress_line, msg),
                  lambda result, err: self.root.after(0, self._finish_training, profile, result, err)),
            daemon=True,
        ).start()

    def _finish_training(self, profile, result, error):
        self.training_running = False
        ...
        self.profile_mgr.record_run(profile, result, make_active=True)
        ...
        ctk.CTkLabel(done_row, text=f"{result['model_key']} training complete for '{profile}'",
        ...
        if examples_dir and os.path.isdir(examples_dir):
            ...
            if example_files:
                ctk.CTkLabel(self.results_body, text="Example heatmaps (red = anomaly strength):",
                             font=self.f_small, text_color=TEXT_MUTED).pack(anchor="w", pady=(0, 4))
                ex_row = ctk.CTkFrame(self.results_body, fg_color="transparent")
                ex_row.pack(fill="x", pady=(0, 12))
                self._example_photos = []
                for fname in example_files[:4]:
                    try:
                        img = Image.open(os.path.join(examples_dir, fname)).convert("RGB")
                        img.thumbnail((100, 100))
                        photo = ctk.CTkImage(light_image=img, dark_image=img, size=img.size)
                        self._example_photos.append(photo)
                        ctk.CTkLabel(ex_row, text="", image=photo).pack(side="left", padx=(0, 8))
                    except Exception:
                        continue
