...
def run_zscore_detection(bgr, rois, window, z_thr, min_area=4.0, min_circularity=0.55,
    ...
    blob, nothing recovered after the fact:
      ...
      2. Direct threshold: a pixel counts as a candidate defect pixel if
         zscore >= z_thr, inside the ROI mask. That's it -- no second
         "weak" tier, no hysteresis chain trying to recover a fainter
         tail by connectivity. A two-tier hysteresis version was tried
         to extend faint thread tails, but it made the whole pipeline
         harder to reason about and wasn't actually the fix that
         mattered -- reverted per direct feedback: keep this simple,
         z-thresholding alone is enough.
      3. A small gap-bridging dilation only (gap_bridge_px, isotropic,
         a few px in any direction) to reconnect a thread broken by a
         couple of noisy/missed pixels. gap_bridge_px=0 means exactly
         zero dilation -- detects precisely what cleared z_thr, nothing
         added.
    ...
    ARC / CONCENTRIC-RING rejection: fits a circle to the blob's own
    contour points (_fit_circle) and checks the fit residual RELATIVE TO
    THE FITTED RADIUS ITSELF (resid / fit_r) -- not relative to the
    blob's own arc-length like an earlier version did. That distinction
    is what makes this correctly reject a full or near-full CONCENTRIC
    RING, not just a short arc segment: a full ring's points sit on a
    circle exactly as tightly as a short arc's points do, but a ring's
    own visible length is roughly 2*pi times its radius -- so a
    length-based ratio test systematically failed on full/near-full
    rings (their "too good a fit for too short a length" reads as
    disqualifying, when for a ring, a big radius-to-length change is
    normal, not suspicious), letting real lens/coating rings leak
    through as thread. Comparing the residual to the fitted radius has
    no such length dependence, so a tiny arc segment and a complete ring
    are judged the same consistent way. fit_r must also be clearly
    bigger than the blob's own width (fit_r >= 2x width_px) so a thread
    that happens to curl into a tight little loop on itself isn't
    mistaken for a much larger lens/coating ring -- a real optical ring's
    radius comes from the lens geometry, not from how tightly a stray
    fiber happened to coil. Given a real thread tracing a clean circle
    by chance is rare while a real lens/coating ring or arc doing so is
    the norm, this is deliberately biased toward calling a clean fit an
    arc/ring over calling it thread.
    ...
    for lc in link_contours:
        ...
        blob_mask = cv2.bitwise_and(raw, region)
        ...
        if circularity >= min_circularity and len(frag_contours) == 1:
            ...
            binary = cv2.bitwise_or(binary, blob_mask)
            continue
    ...
    def _run_pipeline_debug_thread(self, frame, rois_snapshot):
        ...
        try:
            _binary, _blobs, _stats, debug_images = run_zscore_detection(
                ...
                s.get("scale_mm_per_px"), s["min_diameter_mm"],
                s["min_aspect_ratio"], s["max_thin_width_px"], s["max_arc_fit_residual"],
                gap_bridge_px=s["gap_bridge_px"], debug=True)
        except Exception:
            debug_images = None
