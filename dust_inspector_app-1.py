...
def run_zscore_detection(bgr, rois, window, z_thr, min_area=4.0, min_circularity=0.55,
                          ...
                          max_scratch_straightness=0.06, debug=False):
    ...
    Every surviving connected blob is then classified by SHAPE:
      ...
      - elongated (length/width >= min_aspect_ratio), thin
        ...
        a large circle well:
          - and its points DO fit a straight line well          -> SCRATCH
          - otherwise (bends/curves along its length)           -> THREAD/FIBER
    ...
    elongated, NOT a lens/reflection arc) into SCRATCH vs THREAD: a
    scratch is a groove cut into a flat surface, so it is straight by
    construction; a thread is a loose fiber sitting on top of the module,
    free to bend into any curve. Fitting a straight line (not a circle)
    to the same contour points and checking the residual answers exactly
    this -- a low residual relative to length means "traces a straight
    line" (scratch), a high one means "bends along its length" (thread).
    ...
    for lc in link_contours:
        ...
        if width_px > max_thin_width_px:
            ...
            btype = "thread"
            if classify_scratch and len(all_pts) >= 4:
...
class DustInspectorApp:
    ...
    def _build_detection_tab(self, tab):
        ...
        ctk.CTkLabel(card, text="A thin elongated blob is rejected as an arc (lens/coating reflection) only if it cleanly fits ONE circle much bigger than itself -- a real thread can curve too, but essentially never traces a perfect large arc, so curvature alone no longer disqualifies it. Lower the tolerance to be stricter about what counts as a clean circle fit. Of what's left, fitting a STRAIGHT line (not a circle) splits scratch from thread: a low straightness value here means only near-perfectly-straight blobs get called scratch (stricter, more get called thread); raise it to allow gently-curved marks to still count as scratch. Turn off \"Classify scratch\" to fall back to labeling all of these as thread, as before.",
