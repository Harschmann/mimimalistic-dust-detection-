...
Single-file desktop app, restructured into TWO windows:
  ...
  2. Teaching Window (opened via the "Teaching" button) — technician-only
     ...
     camera settings, and a "View Pipeline Steps" debug viewer that shows
     every intermediate image of the last Test Detection run.

Detection algorithm: local Z-score (boxFilter mean/std) thresholded
per-ROI, cleaned up with a morphological opening, then classified into:
  - dust   (compact, round)             -> accepted
  - thread (thin AND short)             -> accepted
  - arc    (thin AND long / sweeping)   -> rejected (lens/reflection artifact)
...
def norm_u8(arr):
    ...
    (the debug step viewer) -- never used in the actual detection math."""
...
def run_zscore_detection(bgr, rois, window, z_thr, min_area=4.0, min_circularity=0.55,
                          ...
                          min_straightness=0.90, return_steps=False):
    ...
    Shape classification: every surviving blob used to be judged by
    circularity alone, which meant a genuine thread/fibre (thin, non-round)
    was rejected exactly like a lens/coating reflection arc (also thin,
    non-round). LENGTH does not reliably tell them apart either -- a real
    thread can be short or long and still just be a thread. What actually
    tells them apart is CURVATURE: a thread runs close to straight (even if
    it has a slight bend), while a lens-ring reflection sweeps a genuinely
    curved arc. So each elongated blob's straightness (chord length / path
    length -- see contour_straightness()) is checked against
    min_straightness:

      - circularity >= min_circularity                 -> "dust"   (accepted)
      - circularity <  min_circularity AND straight     -> "thread" (accepted)
      - circularity <  min_circularity AND curved        -> "arc"    (rejected)

    max_thread_length_px is kept as an optional extra safety cap (default
    effectively disabled) in case you ever want to also reject implausibly
    long straight artifacts.
    ...
    `circles` now also carries a "kind" of "dust" or "thread", plus
    "straightness" for tuning. If return_steps=True, also returns a `steps`
    dict with every intermediate array for the pipeline-steps debug viewer.
    """
    ...
    for c in contours:
        ...
        if circularity >= min_circularity:
            kind = "dust"
        elif straightness >= min_straightness and length_px <= max_thread_length_px:
            kind = "thread"
        else:
            kind = "arc"
    ...
    steps = {
        "raw": bgr, "gray": gray_u8, "local_mean": local_mean, "local_std": local_std,
        "zscore": zscore, "roi_mask": mask, "raw_threshold": raw, "opened": opened,
        "rejected_arc_contours": rejected_arc_contours,
    }
    return binary, circles, stats, steps


def build_pipeline_step_images(rois, circles, steps, params):
    """Turns the raw arrays from run_zscore_detection(..., return_steps=True)
    into a list of (title, image_bgr, caption) tuples, in processing order,
    for the pipeline-steps debug viewer."""
    bgr = steps["raw"]
    gray_u8 = steps["gray"]
    mean_vis = cv2.cvtColor(norm_u8(steps["local_mean"]), cv2.COLOR_GRAY2BGR)
    std_vis = cv2.cvtColor(norm_u8(steps["local_std"]), cv2.COLOR_GRAY2BGR)

    z_vis = np.clip((steps["zscore"] + 4) / 8 * 255, 0, 255).astype(np.uint8)
    z_color = cv2.applyColorMap(z_vis, cv2.COLORMAP_JET)
    ...
    raw_overlay[steps["raw_threshold"] == 255] = (0, 255, 255)

    opened_overlay = bgr.copy()
    opened_overlay[steps["opened"] == 255] = (0, 255, 255)

    classify_vis = bgr.copy()
    ...
    n_defects = len(circles)
    verdict = "FAIL" if n_defects else "PASS"
    vcolor = (0, 0, 255) if n_defects else (0, 200, 0)
    cv2.putText(final, f"Verdict: {verdict}  (defects={n_defects})", (14, 30),
                cv2.FONT_HERSHEY_SIMPLEX, 0.8, vcolor, 2, cv2.LINE_AA)
    ...
    return [
        ...
        ("2. Grayscale", cv2.cvtColor(gray_u8, cv2.COLOR_GRAY2BGR),
         "cv2.cvtColor(BGR2GRAY) -- intensity only, color dropped."),
        ("3. Local Mean \u03bc", mean_vis,
         f"cv2.boxFilter, window={w}px. Smooths out lighting/shading."),
        ("4. Local Std \u03c3", std_vis,
         "sqrt(boxFilter(I\u00b2) \u2212 \u03bc\u00b2). Local texture/variation map."),
        ("5. Z-score Map", z_color,
         f"Z = |I \u2212 \u03bc| / \u03c3. Threshold = {zthr}."),
        ("6. ROI", roi_vis, "Only pixels inside the circular ROI(s) are evaluated."),
        ("7. Threshold + ROI", raw_overlay,
        ...
        ("9. Dust / Thread / Arc", classify_vis,
         f"circularity\u2265{min_circ} \u2192 dust (green). Elongated & straightness\u2265{min_straight} \u2192 "
         f"thread (yellow, even if long). Elongated & curved \u2192 arc, rejected (red)."),
        ("10. Verdict", final,
...
class DustInspectorApp:
    def __init__(self, root):
        ...
        self.last_circles = []         # last inspection's accepted dust/thread blobs
        self.inspection_running = False

        # debug: last Test Detection's intermediate pipeline steps (Teaching only)
    ...
    def _build_main_overlay(self):
        ...
        for d in self.last_circles:
            ...
            cv2.putText(disp, label, (cx + r + 10, cy + 8), cv2.FONT_HERSHEY_SIMPLEX, 0.9, color, 3, cv2.LINE_AA)
        return disp
    ...
    def _finish_inspection(self, frame, rois_snapshot, circles, barcode):
        ...
        # save source (with ROI overlay) + verdict-filed result (with rings)
    ...
    def _build_roi_tab(self, tab):
        ...
        self._btn_secondary(top, "Test Detection", self.test_detection_once, width=130).pack(side="right", padx=(6, 0))
        self.view_steps_btn = self._btn_secondary(top, "View Pipeline Steps", self.open_pipeline_steps_window, width=160)
        self.view_steps_btn.pack(side="right", padx=(6, 0))
        self.view_steps_btn.configure(state="disabled")
    ...
    def test_detection_once(self):
        ...
        binary, circles, stats, steps = run_zscore_detection(
            ...
            s.get("scale_mm_per_px"), s["min_diameter_mm"], s.get("max_thread_length_px", 100000.0),
            s.get("min_straightness", 0.90), return_steps=True)
        self.last_circles = circles
        ...
        if stats:
            ...
            msg = "no detection (empty ROI/mask)"
        self.footer_var.set("Test detection: " + msg)

    # ---- pipeline steps debug viewer -----------------------------------
    def open_pipeline_steps_window(self):
        if self.last_steps is None:
            messagebox.showinfo("Pipeline Steps", "Run Test Detection first.")
            return
        ...
        win.title("Pipeline Steps — Debug Viewer")
        win.geometry("1180x760")
        win.configure(fg_color=BG)

        state = {"i": len(step_images) - 1, "photo": None}
        ...
        left = self._card(body, width=200)
        left.grid(row=0, column=0, sticky="nsew", padx=(0, 12))
        left.grid_propagate(False)
        ctk.CTkLabel(left, text="STEPS", font=self.f_small, text_color=TEXT_MUTED).pack(anchor="w", padx=14, pady=(14, 6))
        steps_scroll = ctk.CTkScrollableFrame(left, fg_color="transparent")
        steps_scroll.pack(fill="both", expand=True, padx=6, pady=(0, 10))
        ...
        canvas_card = self._card(right)
        canvas_card.grid(row=0, column=0, sticky="nsew")
        wrap = ctk.CTkFrame(canvas_card, fg_color=BG_CANVAS, corner_radius=10)
        wrap.pack(fill="both", expand=True, padx=14, pady=(14, 6))
        canvas = tk.Canvas(wrap, bg=BG_CANVAS, highlightthickness=0)
        canvas.pack(fill="both", expand=True, padx=3, pady=3)

        nav = ctk.CTkFrame(canvas_card, fg_color="transparent")
        nav.pack(fill="x", padx=14, pady=(0, 6))
        title_var = tk.StringVar(value="")
        ctk.CTkButton(nav, text="\u2190 Prev", width=90, corner_radius=8, fg_color=BG_CARD_ALT,
                      hover_color=BORDER, text_color=TEXT, command=lambda: go(-1)).pack(side="left")
        ctk.CTkLabel(nav, textvariable=title_var, font=self.f_section, text_color=TEXT).pack(side="left", padx=16)
        ctk.CTkButton(nav, text="Next \u2192", width=90, corner_radius=8, fg_color=BG_CARD_ALT,
                      hover_color=BORDER, text_color=TEXT, command=lambda: go(1)).pack(side="left")

        caption_var = tk.StringVar(value="")
        ctk.CTkLabel(canvas_card, textvariable=caption_var, font=self.f_body, text_color=TEXT_MUTED,
                     wraplength=760, justify="left").pack(anchor="w", padx=14, pady=(0, 14))

        def render():
            canvas.delete("all")
            title, img, caption = step_images[state["i"]]
            title_var.set(title)
            ...
            for i, b in enumerate(step_buttons):
                b.configure(fg_color=ACCENT if i == state["i"] else BG_CARD_ALT,
                            text_color="#ffffff" if i == state["i"] else TEXT)

        def go(delta):
            state["i"] = max(0, min(len(step_images) - 1, state["i"] + delta))
            render()
        ...
        step_buttons = []
        for i, (title, img, caption) in enumerate(step_images):
            b = ctk.CTkButton(steps_scroll, text=title, anchor="w", corner_radius=8,
                               fg_color=BG_CARD_ALT, hover_color=BORDER, text_color=TEXT,
                               font=self.f_body, command=lambda i=i: select(i))
            b.pack(fill="x", padx=4, pady=3)
            step_buttons.append(b)
