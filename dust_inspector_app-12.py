...
def run_zscore_detection(bgr, rois, window, z_thr, min_area=4.0, min_circularity=0.55,
                          ...
                          gap_bridge_px=7.0, max_concentric_band_cv=0.12, debug=False):
    """Deliberately SIMPLE pipeline -- one threshold, one shape pass per
    blob, nothing recovered after the fact:

      1. Local Z-score via boxFilter mean/std, same as always.
      2. Direct threshold: a pixel counts as a candidate defect pixel if
         zscore >= z_thr, inside the ROI mask. No weak/hysteresis tier.
      3. A small gap-bridging dilation only (gap_bridge_px, isotropic, a
         few px in any direction). gap_bridge_px=0 means exactly zero
         dilation -- detects precisely what cleared z_thr, nothing added.
      4. Each surviving blob classified by shape:
           - round + big + circular enough, one solid contour -> DUST
           - elongated (length/width >= min_aspect_ratio), thin
             (width <= max_thin_width_px), not a lens/ring reflection -> THREAD
           - elongated + thin AND matches a circle/ring (see below)   -> ARC, rejected
           - elongated + wide (width > max_thin_width_px)             -> GLUE
         Width uses the 70th percentile of local half-width (distance
         transform), not the raw max, so one locally thicker patch on an
         otherwise-thin thread doesn't single-handedly decide the call.

    PERFORMANCE: blobs are found via connectedComponentsWithStats (one
    vectorized call) instead of looping findContours per blob and
    redrawing it into a full-frame-sized mask each time. A dilated/
    linked component's own area (from that single stats call) is always
    >= the raw-intersected area for that region -- dilation only adds
    pixels -- so components that don't even clear min_area at the
    (larger) dilated size are filtered out with one array comparison
    over ALL components at once, before any per-blob work happens.
    Whatever survives that is then processed on a small crop of its own
    bounding box, not the whole frame. This is what was making a loose
    z_thr slow: a loose threshold can produce thousands of tiny noise
    fragments, and the old per-blob full-frame allocate-and-redraw cost
    was being paid for every single one of them.

    ARC / CONCENTRIC-RING / CRESCENT rejection: two checks now, since a
    lighting/lens artifact doesn't always look like a clean thin arc --
      1. Circle fit (_fit_circle) on the blob's own contour points, with
         the residual checked RELATIVE TO THE FITTED RADIUS ITSELF
         (resid / fit_r), not relative to the blob's own length -- this
         is what correctly rejects a full/near-full ring as well as a
         short arc segment (a length-based ratio systematically failed
         on full rings, see prior version's notes). fit_r must also be
         clearly bigger than the blob's own width (fit_r >= 2x width_px)
         so a thread curled into a tight loop on itself isn't mistaken
         for lens-scale curvature. Only checked for THIN blobs (this is
         a shape-fit test).
      2. Concentric-band check using the ROI's own known center: for
         every point in the blob, distance from the ROI center is
         computed, and if those distances cluster tightly (coefficient
         of variation <= max_concentric_band_cv) AND the points sweep a
         meaningful angle around that center (>= 25 degrees, not just a
         narrow wedge), the blob is rejected regardless of width. Both
         conditions matter: distance-clustering alone isn't enough,
         since ANY short segment not pointed straight at/away from the
         center barely changes distance across its own short length --
         a short, ordinary off-center thread can trigger a low CV purely
         by being short, not by actually curving around the center. The
         angular-sweep requirement is what tells a genuine ring/arc
         apart from an unrelated short thread that just happens to sit
         at a roughly constant radius. This combination is what a
         "moon"/crescent-shaped reflection near the lens center needs:
         it has two boundary edges at DIFFERENT radii (an annular
         sector, not one clean arc), so a single circle fit doesn't
         explain it well -- but it's still obviously centered on the
         lens axis and sweeps a real angle, which real contamination has
         no reason to do. Checked BEFORE the glue/thread width split,
         since a crescent artifact can be wide enough to otherwise be
         routed straight to glue without ever being shape-tested at all.
    Given a real thread tracing a circle or sitting in a tight radial
    band around the lens center by chance is rare, while a real lens/
    coating artifact doing either is the norm, both checks are
    deliberately biased toward rejecting a clean match rather than
    calling it thread.
    ...
    pipeline images for the pipeline-steps viewer.
    """
    ...
    # Closing (dilate then erode) is unreliable here: the erode half
    # removes a thin bridge just as easily as it would remove the
    # thread's own thin width. Dilating (no erode) reliably links nearby
    # fragments; shape is then measured from the ORIGINAL undilated
    # pixels within each linked region, so linking doesn't inflate the
    # measured width. gap_bridge_px=0 means EXACTLY zero dilation -- no
    # forced minimum iteration.
    small_kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3))
    iterations = max(0, int(round(gap_bridge_px / 2.0)))
    linked = raw if iterations <= 0 else cv2.dilate(raw, small_kernel, iterations=iterations)

    H, W = raw.shape
    num_labels, labels, stats_cc, _centroids = cv2.connectedComponentsWithStats(linked, connectivity=8)
    blobs = []
    binary = np.zeros_like(raw)
    rejected = 0

    if num_labels > 1:
        areas_cc = stats_cc[1:, cv2.CC_STAT_AREA]
        candidate_labels = (np.nonzero(areas_cc >= min_area)[0] + 1).tolist()
        rejected += int(num_labels - 1 - len(candidate_labels))
    else:
        candidate_labels = []
    ...
    for lbl in candidate_labels:
        ...
        blob_mask = np.where(local_labels == lbl, local_raw, 0).astype(np.uint8)
        area = int(cv2.countNonZero(blob_mask))
        if area < min_area:
            rejected += 1
            continue

        frag_contours, _ = cv2.findContours(blob_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
        ...
        if len(frag_contours) == 1:
            peri_c = cv2.arcLength(frag_contours[0], True)
            circularity = (4 * np.pi * area / (peri_c * peri_c)) if peri_c > 0 else 0.0
        else:
            circularity = 0.0  # a linked group of several disconnected fragments is never one dust speck

        dist = cv2.distanceTransform(blob_mask, cv2.DIST_L2, 5)
        ys, xs = np.nonzero(blob_mask)
        if ys.size >= 10:
            # 70th percentile of local half-width, not the raw maximum --
            # a short, locally thicker patch (e.g. a knot on an otherwise
            # thin thread) could otherwise single-handedly push width_px
            # past max_thin_width_px even though most of the object's
            # length is genuinely thin.
            width_px = 2.0 * float(np.percentile(dist[ys, xs], 70))
        else:
            width_px = 2.0 * float(dist.max()) if ys.size else 0.0

        if circularity >= min_circularity and len(frag_contours) == 1:
            # only a genuinely solid single blob counts as dust
            (cx_l, cy_l), r = cv2.minEnclosingCircle(frag_contours[0])
            cx, cy = cx_l + x0, cy_l + y0
            diameter_px = 2.0 * r
            ...
            binary[y0:y1, x0:x1] = cv2.bitwise_or(binary[y0:y1, x0:x1], blob_mask)
            continue
        ...
        all_pts_local = np.vstack([fc.reshape(-1, 2) for fc in frag_contours]).astype(np.float64)
        all_pts = all_pts_local + np.array([x0, y0])
        rcx, rcy = float(all_pts[:, 0].mean()), float(all_pts[:, 1].mean())
        r_draw = max(length_px / 2.0, 3.0)

        # Concentric ring/arc/crescent rejection using the ROI's own
        # known center (see docstring). Checked before the width split
        # since a crescent artifact can be wide.
        #
        # The distance-CV check alone isn't enough: ANY short segment
        # that isn't pointed straight at/away from the center barely
        # changes distance across its own short length, purely because
        # it's short -- not because it's actually curving around the
        # center. A short, fairly straight thread sitting off-center can
        # trigger a low CV by that coincidence alone. A real ring/arc/
        # crescent artifact doesn't just sit near-equidistant -- it
        # actually SWEEPS a meaningful angle around the center. Requiring
        # both is what tells a genuine ring apart from an ordinary
        # off-center thread that just happens to be short.
        MIN_ARC_SPAN_DEG = 25.0
        nearest_roi, nearest_d = None, None
        for roi in rois:
            d = float(np.hypot(rcx - roi["cx"], rcy - roi["cy"]))
            if nearest_d is None or d < nearest_d:
                nearest_d, nearest_roi = d, roi
        is_concentric_artifact = False
        if nearest_roi is not None:
            dx = all_pts[:, 0] - nearest_roi["cx"]
            dy = all_pts[:, 1] - nearest_roi["cy"]
            dists = np.hypot(dx, dy)
            mean_d = float(dists.mean())
            if mean_d > 1e-6 and float(dists.std() / mean_d) <= max_concentric_band_cv:
                angles = np.degrees(np.arctan2(dy, dx))
                sorted_angles = np.sort(angles)
                gaps = np.diff(sorted_angles)
                wrap_gap = 360.0 - (sorted_angles[-1] - sorted_angles[0])
                largest_gap = max(float(gaps.max()) if gaps.size else 0.0, wrap_gap)
                angular_span = 360.0 - largest_gap
                if angular_span >= MIN_ARC_SPAN_DEG:
                    is_concentric_artifact = True
        if is_concentric_artifact:
            rejected += 1
            continue
        ...
        if width_px > max_thin_width_px:
            ...
            if len(all_pts) >= 8:
                ...
                if fit_r is not None and fit_r > 1e-6:
                    norm_resid = resid / fit_r  # relative to the circle's OWN size --
                                                 # works the same for a short arc or a full ring
                    if norm_resid <= max_arc_fit_residual and fit_r >= 2.0 * max(width_px, 1.0):
                        is_arc = True
            if is_arc:
                rejected += 1
                continue  # a clean fit to a circle clearly bigger than the blob's own
                          # width -- a real optical reflection/ring (fixed lens
                          # geometry), not contamination
            btype = "thread"
...
class DustInspectorApp:
    def __init__(self, root):
        ...
        # trimmed down from the original sizing so the page fits on a laptop
        # screen without scrolling, not just a full monitor
        self.f_op_title = ctk.CTkFont(size=32, weight="bold")
        self.f_op_subtitle = ctk.CTkFont(size=16)
        self.f_op_label = ctk.CTkFont(size=14, weight="bold")
        self.f_op_status = ctk.CTkFont(size=36, weight="bold")
        self.f_op_barcode = ctk.CTkFont(size=20)
        self.f_op_button = ctk.CTkFont(size=22, weight="bold")
        self.f_op_button_sm = ctk.CTkFont(size=15, weight="bold")
        self.f_op_counter_num = ctk.CTkFont(size=26, weight="bold")
        self.f_op_stat_num = ctk.CTkFont(size=32, weight="bold")
        ...
        self.feed_frozen = False       # True from Start until Resume Feed is pressed
        self.showing_result_strip = False  # True after inspection finishes, until Resume Feed
    ...
    def _build_operator_page(self, page):
        ...
        feed_btn_row = ctk.CTkFrame(right, fg_color="transparent")
        feed_btn_row.pack(fill="x", pady=(0, 8))
        feed_btn_row.grid_columnconfigure(0, weight=1)
        feed_btn_row.grid_columnconfigure(1, weight=1)
        self._btn_secondary(feed_btn_row, "Resume Feed", self.resume_feed,
                             font=self.f_op_button_sm, height=40).grid(row=0, column=0, sticky="ew", padx=(0, 4))
        self._btn_secondary(feed_btn_row, "Clear Markings", self.clear_detection_markings,
                             font=self.f_op_button_sm, height=40).grid(row=0, column=1, sticky="ew", padx=(4, 0))
    ...
    def clear_detection_markings(self):
        ...
        self.last_blobs = []
        self.showing_result_strip = False
    ...
    def _poll_live(self):
        if not self.using_static_image and not self.feed_frozen:
    ...
    def _build_main_overlay(self):
        """Plain live/idle/frozen view -- deliberately shows NO ROI
        indicator at all, only defect markers from the last inspection
        (if any). ROI squares are shown ONLY in the result strip after an
        inspection finishes (_build_result_strip), not here -- showing
        them at all times on the operator feed was a mistake: they'd
        appear the moment the app opens (loaded from the saved ROI
        layout) and "Clear Markings" couldn't remove them since they were
        never inspection markings, just the ROI setup itself."""
    ...
    def _build_result_strip(self):
        """Inspection-result view: crops just each ROI's padded square out
        of the frozen frame (not the whole frame with tiny squares on it),
        draws that ROI's own defect markers translated into the crop's
        local coordinates, and lays every crop side by side left to
        right -- so each inspected zone is shown bigger and clearly,
        instead of small within the full frame. Shown from the moment an
        inspection finishes until Resume Feed is pressed, which goes back
        to the normal full live view."""
        margin = self.settings.get("roi_square_margin_px", 20.0)
        ...
        for roi in self.rois:
            ...
            if x1c <= x0c or y1c <= y0c:
                continue
            crop = self.original[y0c:y1c, x0c:x1c].copy()
            for b in self.last_blobs:
                bcx, bcy, br = b["cx"], b["cy"], b["r"]
                if not (x0c <= bcx <= x1c and y0c <= bcy <= y1c):
                    continue
                lcx, lcy = int(round(bcx - x0c)), int(round(bcy - y0c))
                lr = int(round(br))
                color = BLOB_COLOR_BGR.get(b["type"], (0, 0, 255))
                cv2.circle(crop, (lcx, lcy), lr + 4, color, 2)
                cv2.putText(crop, b["label"], (lcx + lr + 6, lcy + 6),
        ...
        for c in crops:
            h, w = c.shape[:2]
            scale = target_h / h
            resized.append(cv2.resize(c, (max(1, int(round(w * scale))), target_h)))
        total_w = sum(r.shape[1] for r in resized) + gap * max(0, len(resized) - 1)
        strip = np.zeros((target_h, total_w, 3), dtype=np.uint8)
        x = 0
        for r_img in resized:
            w = r_img.shape[1]
            strip[:, x:x + w] = r_img
            x += w + gap
        return strip
    ...
    def _get_display_image(self):
        if self.original is None:
            return None
        if self.showing_result_strip:
            return self._build_result_strip()
        return self._build_main_overlay()
    ...
    def _append_log_line(self, ts, barcode, model, line, verdict, dust_count, max_dia, findings_text):
        ...
        line_txt = (f"[{ts}] {barcode:<16} | Model:{model or '-':<10} Line:{line or '-':<8} | "
                    f"{verdict:<5} | {findings_text} | max={dia_txt}\n")
    ...
    def start_inspection(self):
        ...
        self.feed_frozen = True  # stop the displayed feed from updating until Resume Feed is pressed
        self.showing_result_strip = False  # show the plain frozen frame while this inspection runs
    ...
    def resume_feed(self):
        """Un-freezes the operator feed so it starts following the live
        camera again, and switches back from the result-strip view (if
        showing) to the normal full frame. Also exits static-image mode
        (Open Image) -- without this, using_static_image stayed True
        forever after loading a static image, and _poll_live checks BOTH
        flags, so clearing feed_frozen alone was never enough to get the
        live feed moving again once an image had been loaded. Clears the
        last inspection's defect markers too -- resuming to a live feed
        should be a clean slate, not carry over stale markers from
        ...
        self._set_status("IDLE", TEXT_MUTED)
        self._render_main_feed()
        self.footer_var.set("Feed resumed -- showing live camera.")
    ...
    def _run_inspection_thread(self, frame, rois_snapshot, barcode):
        ...
        try:
            _binary, blobs, _stats, _dbg = run_zscore_detection(
                ...
                gap_bridge_px=s["gap_bridge_px"], max_concentric_band_cv=s.get("max_concentric_band_cv", 0.12))
        except Exception:
            blobs = []
    ...
    def _save_inspection_artifacts(self, frame, rois_snapshot, blobs, barcode):
        ...
        # Source image saved EXACTLY as the camera captured it -- no ROI
        # or defect overlay drawn on it. This is the archival/raw record
        # (useful for re-running detection later, or auditing what the
        # camera actually saw); the ROI overlay belongs only on the
        # NG/OK result image below, not baked into the source copy.
        cv2.imwrite(os.path.join(SOURCE_DIR, f"source_{ts}_{barcode}.png"), frame)

        result_disp = frame.copy()
        for roi in rois_snapshot:
            cv2.circle(result_disp, (int(roi["cx"]), int(roi["cy"])), int(roi["r"]), (0, 255, 0), 2)
        max_dia = None
        for b in blobs:
            cx, cy, r = int(b["cx"]), int(b["cy"]), int(round(b["r"]))
            color = BLOB_COLOR_BGR.get(b["type"], (0, 0, 255))
        ...
        findings_text = self._build_findings_text(blobs)
        log_args = (log_ts, barcode, model, line, verdict, len(blobs), max_dia, findings_text)
    ...
    def _build_findings_text(self, blobs):
        ...
        parts = [f"{counts[t]}x {t}" for t in ("dust", "thread", "glue") if counts.get(t)]
        return ", ".join(parts)
    ...
    def _finish_inspection(self, blobs, verdict, log_args):
        ...
        self.showing_result_strip = True  # show cropped/enlarged ROI squares, not the tiny full frame
    ...
    def _build_roi_tab(self, tab):
        ...
        display_card = self._card(side)
        display_card.pack(fill="x", pady=(10, 0))
        ctk.CTkLabel(display_card, text="Operator Display", font=self.f_section, text_color=TEXT).pack(anchor="w", padx=16, pady=(14, 6))
        ctk.CTkLabel(display_card, text="On the Operator feed, each ROI is shown as a square with this much margin around it (not the raw circle) -- so the operator sees a clean inspection zone, not calibration detail.",
                     font=self.f_small, text_color=TEXT_MUTED, wraplength=280, justify="left").pack(anchor="w", padx=16, pady=(0, 8))
        margin_row = ctk.CTkFrame(display_card, fg_color="transparent")
        margin_row.pack(fill="x", padx=16, pady=(0, 16))
        self.roi_square_margin_var = tk.StringVar(value=str(self.settings.get("roi_square_margin_px", 20.0)))
        self._field(margin_row, "Square margin (px)", self.roi_square_margin_var, width=90).pack(side="left")
        self._btn_primary(margin_row, "Save", self._save_display_settings, width=80).pack(side="left", padx=(10, 0), pady=(18, 0))

    def _save_display_settings(self):
        try:
            self.settings["roi_square_margin_px"] = float(self.roi_square_margin_var.get())
        except ValueError:
            messagebox.showerror("Settings", "Square margin must be a number.")
            return
        save_settings(self.settings)
        self._render_main_feed()
        self.footer_var.set("Operator display settings saved.")
    ...
    def _run_pipeline_debug_thread(self, frame, rois_snapshot):
        ...
        try:
            _binary, _blobs, _stats, debug_images = run_zscore_detection(
                ...
                s.get("scale_mm_per_px"), s["min_diameter_mm"],
                s["min_aspect_ratio"], s["max_thin_width_px"], s["max_arc_fit_residual"],
                gap_bridge_px=s["gap_bridge_px"], max_concentric_band_cv=s.get("max_concentric_band_cv", 0.12), debug=True)
        except Exception:
            debug_images = None
    ...
    def _build_detection_tab(self, tab):
        ...
        self._field(row3, "Arc/ring-fit tolerance (0-1)", self.arc_fit_var, width=100).pack(side="left", padx=(0, 16))
        ctk.CTkLabel(card, text="A thin elongated blob is rejected as an arc/ring (lens or coating reflection) only if it cleanly fits a circle clearly bigger than the blob's own width -- this catches a short arc segment and a full/near-full concentric ring the same way, so real lens/coating rings are rejected, not just partial arcs. A real thread essentially never traces a clean circle by chance, so this leans toward calling a clean fit an arc/ring over calling it thread. Lower the tolerance to be stricter about what counts as a clean circle fit.",
        ...
        self.concentric_band_var = tk.StringVar(value=str(self.settings.get("max_concentric_band_cv", 0.12)))
        self._field(row4, "Gap bridge (px)", self.gap_bridge_var, width=90).pack(side="left", padx=(0, 16))
        self._field(row4, "Ring/crescent band tol. (0-1)", self.concentric_band_var, width=130).pack(side="left", padx=(0, 16))
        self._btn_primary(row4, "Save", self._save_detection_settings, width=100).pack(side="left", pady=(18, 0))
        ctk.CTkLabel(card, text="Detection is a single direct z-score threshold (z_thr above) -- no second weak/hysteresis tier. Gap bridge only reconnects pieces separated by a true GAP with no signal at all, over a short distance, any direction. Set gap bridge to 0 for zero dilation -- detects exactly what clears the z-score threshold, nothing added. Ring/crescent band tolerance rejects any blob (thin OR wide) whose points sit in a tight radial band around the ROI's own center -- catches moon/crescent-shaped lighting reflections near the lens center, which a simple circle fit alone can miss since a crescent has two different-radius edges. Lower it to be stricter (fewer false rejections of real contamination that happens to sit near center); raise it if a crescent/ring is still slipping through as a defect.",
    ...
    def _save_detection_settings(self):
        try:
            ...
            self.settings["max_concentric_band_cv"] = float(self.concentric_band_var.get())
        except ValueError:
